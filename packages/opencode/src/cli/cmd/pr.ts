...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: `fetch and checkout a GitHub PR branch, then run ${CLI}`,
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found ${PRODUCT} session: ${sessionUrl}`)
    ...
    UI.println()
    UI.println(`Starting ${PRODUCT}...`)
    UI.println()

    const opencodeArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn([CLI, ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`${CLI} exited with code ${code}`))
  }),
})
