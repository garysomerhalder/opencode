...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const invalidate = Effect.fn("Config.invalidate")(function* () {
      ...
      // Drop the per-instance snapshot so the next get() reloads from disk.
      // Without this, config-file edits (plugin/MCP manager mutations) stay
      // invisible to status() and friends until the instance is disposed.
      // Skipped when no instance is in scope (CLI/test harnesses without one).
      const ctx = yield* InstanceRef
      if (ctx) yield* InstanceState.invalidate(state)
    })
