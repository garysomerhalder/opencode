...
import { PRODUCT, SHORT } from "@opencode-ai/core/brand"
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle(PRODUCT)
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`${SHORT} | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`${SHORT} | ${route.data.id}`)
    }
  })
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to ${PRODUCT} v${result.data.version}. Please restart the application.`,
    )
